...
class CourseInfoForm(FlaskForm):
    ...
    def validate_grade_and_currently_enrolled(self, grade,currently_enrolled):              # checks for duplicate EMPLID's 
        enrollement = Enrollement.query.filter_by(
                            student_id=current_user.EMPLID,
                            course_id = course.id).first()
        if grade.data and enrollement.currently_enrolled != False:
            raise ValidationError('That EMPLID is already in use!')

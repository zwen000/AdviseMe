...
# faculty can go editing the direct advising note in this route
# @app.route('/academic/review/<int:note_id>', methods=['GET', 'POST'])
# @app.route('/academic_advisor/review/', methods=['GET', 'POST'])
# @login_required
# # def academic_review(note_id):
# def academic_review():
#     form = AcademicReviewForm()
#     student_id = 233967
#     notes = Notes.query.filter(Student.EMPLID == student_id).first()
#     student = Student.query.filter_by(EMPLID=student_id).first()
#     return render_template('FacultyAdvisorReview.html', form=form, notes=notes, student=student)